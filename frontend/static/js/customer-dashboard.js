...
async function renderAllBookings(filter = 'all') {
  ...
  // Show / hide Review column based on filter
                ...
                if (b.review) {
                    reviewCell = `<td>
                      <div style="display:inline-flex;align-items:center;gap:6px;padding:5px 12px;
                        border-radius:8px;background:rgba(52,211,153,0.12);border:1.5px solid rgba(52,211,153,0.3)">
                        <span style="color:#34d399;font-size:13px">✓</span>
                        <span style="color:#059669;font-size:12px;font-weight:600">Submitted</span>
                        <span style="color:#f59e0b;font-size:12px">${'★'.repeat(b.review.rating)}</span>
                      </div>
                    </td>`;
                  } else {
                  reviewCell = `<td>
